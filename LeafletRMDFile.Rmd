...
Try to work through down this script, observing what happens in the plotting pane. There are three initial example exercises, followed by a few questions and 5 tasks. 
...
Answer: It determines whether the box in the top right is collapsable or not.
...
If you experience difficulty with your read_sheet() function (it is erroring out), uncomment and run the following function: gs4_deauth()  # run this line and then rerun the read_sheet() function below
...
sights = c("Randers Regnskov", "Dolce Vita", "Oyisi Sushi")

leaflet() %>%                                 
  addProviderTiles("Esri.WorldPhysical") %>%  
  addAwesomeMarkers(lng = c(10.0299577, 10.199354, 10.1944196),      
                    lat = c(56.457126, 56.1802838, 56.1943684),      
                    popup = sights)
...
## Your brief answer:
The clustered map would be a good way to see where there are the most interesting sights in Denmark fx. if you were planning a vacation with friends/family.
The un-clustered map becomes very cluttered with points at the more busy areas, but might be preferable with fewer points.
...
## Solution
```{r eval = TRUE}
circles <- data.frame(lng = c(10.0299577, 10.199354), lat = c(56.457126, 56.1802838))

DANmap %>%
  addTiles() %>%
  setView(10.2, 56.16, zoom = 12) %>%
  addCircles(lng = places$Longitude, 
             lat = places$Latitude, radius = places$Stars1_5*10,
             popup = paste0(places$Placename, "<hr>", places$Type, "<br>", places$Description))
```
